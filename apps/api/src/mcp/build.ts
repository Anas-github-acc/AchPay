/**
 * The MCP server for the agent-ready storefront: the tools themselves.
 *
 * A factory rather than a module-level singleton, because the two transports
 * want different lifetimes. Over stdio there is one process, one server and
 * one client for as long as it runs. Over HTTP a server binds to exactly one
 * transport, so each session gets its own pair — see http/mcp-route.ts.
 *
...
import { z } from 'zod';
import { apiGet, apiPost, isFailure, query, type ApiFailure, type ApiResponse } from './api.js';
...
/** Builds a server with every tool registered. Connect it to a transport. */
